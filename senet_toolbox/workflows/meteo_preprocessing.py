...
from meteo_utils import ecmwf_utils as eu, solar_irradiance as sun
...
def process_single_date(
    ...
    template_file: str
        Path to GDAL compatible template image to which data will be resampled at the end
    date_int : int or str
    ...
    del fid
    date_ini = datetime - dt.timedelta(1)
    date_end = datetime + dt.timedelta(1)
    ...
    print(
        ...
        f"..and dates {date_ini} to {date_end}"
    )
    ...
    cds_target = dst_folder / f"{datetime:%Y%m%d}_era5.grib"
    ...
    if datetime.year == dt.date.today().year:
        dataset = "cams-global-atmospheric-composition-forecasts"
    else:
        dataset = "cams-global-reanalysis-eac4"

    ads_target = dst_folder / f"{datetime:%Y%m%d}_cams.grib"
    ...
    print(f"Processing ECMWF data for UTC time {datetime}\nThis may take some time...")
    ...
    if dst_folder:
        for param, array in output.items():
            if param not in DAILY_VARS:
                if param == "SW-IN":
                    for i, var1 in enumerate(["DIR", "DIF"]):
                        for j, var2 in enumerate(["PAR", "NIR"]):
                            param = f"{var2}-{var1}"
                            filename = f"{datetime:%Y%m%dT%H%M%S}_{param.upper()}.tif"
                            ...
                            resample_to_template(ds, dst_file, template_file)
                else:
                    filename = f"{datetime:%Y%m%dT%H%M%S}_{param.upper()}.tif"
                    dst_file = str(dst_folder / filename)
                    print(f"Saving {param} to {dst_file}")
                ...
                filename = f"{datetime:%Y%m%d}_{param.upper()}.tif"
                dst_file = str(dst_folder / filename)
                print(f"Saving {param} to {dst_file}")
                driver = gdal.GetDriverByName("MEM")
                dims = array.shape
...
def get_meteo_data(
    ...
    logging.info("Summing irradiance components.")
    di_files = list(Path(out_dir_meteo).glob("*-DI*"))
    if not di_files:
        logging.warning("No irradiance components files found for summation.")
        return None
    ...
    # Save final output
    save_file = os.path.join(out_dir_meteo, file.stem.split("_")[0] + "_SW-IN.tif")
    sum_image.rio.to_raster(save_file)
    resample_to_template(save_file, save_file, template_path)
